<template>
    <div class="mt-14">
        ...
        <p class="remark-cities w-4/5 mx-auto md:text-sm text-xs">Nous vous fournissons les cadeaux les moins chers qui ne dépassent pas</p>
        ...
        <div class="w-4/5 mx-auto flex flex-row-reverse">
            <router-link to="/search">
               <button class="btn-see-more-moin">VOIR PLUS</button>
            </router-link>
        </div>
    </div>
</template>
  ...
  created(){
   ...
   methods:{
        async getProductsPlus(){
            ...
            axios.get('http://localhost/hadiyati/cheap-products').then((response) => {
...
.btn-see-more-moin:hover,
.btn-see-more-moin:focus-visible {
  --_p: 0%;
  --_i: 1;
}

.btn-see-more-moin:active {
  text-shadow: none;
  color: var(--c);
